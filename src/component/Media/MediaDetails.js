                ...
                <div className='max-w-lg m-auto'>
                    <div className="flex flex-col  p-6 space-y-6 overflow-hidden rounded-lg shadow-md dark:bg-gray-900 dark:text-gray-100">
                        <div className="flex space-x-4">
                            <img alt="" src={ mediaDetails?.profileImg || user?.photoURL  ? mediaDetails?.profileImg || user?.photoURL : "N/I"}  className="object-cover w-12 h-12 rounded-full shadow dark:bg-gray-500" />
                            <div className="flex flex-col space-y-1">
                                <Link rel="noopener noreferrer" href="#" className="text-sm font-semibold">{ mediaDetails?.name || user?.displayName ? mediaDetails?.name || user?.displayName: 'name is null'}</Link>
                                <span className="text-xs dark:text-gray-400">{mediaDetails?.job ? mediaDetails?.job :'Web Developer || React Developer ||'}</span>
                            </div>
                        </div>
                        <div>
                        ...
                        <div className="flex flex-wrap justify-between">
                            <div className="flex space-x-2 text-sm dark:text-gray-400">
                                <button type="button" className="flex items-center p-1 space-x-1.5">
                                    <svg onClick={() => handlerLikeClick(`${mediaDetails?._id}`)} style={{backgroundColor: likeClick? 'blue' : 'white'}} xmlns="http://www.w3.org/2000/svg" viewBox="0 0 512 512" aria-label="Number of likes" className="w-5 h-5 border rounded-full  fill-current dark:text-violet-400">
                        ...
                        <div className='flex justify-center'>
                            <Link to={'/media'}>
                                <button className='btn btn-sm btn-info text-white'>Back Page</button>
                            </Link>
                        </div>
                    </div>
                </div>
